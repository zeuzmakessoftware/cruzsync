 ...
 */
const KEY_QUALIFIERS = new Set(['api', 'secret', 'private', 'access', 'client', 'signing']);
...
function isSensitiveKey(name: string): boolean {
  ...
  return ['apikey', 'authtoken', 'accesstoken', 'secretkey', 'privatekey'].some((s) =>
    flat.includes(s),
  );
}

const FREE_TEXT_KEYS = new Set(['message', 'query', 'transcript', 'note', 'utterance']);
...
export function redactArgs(args: unknown): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(args as Record<string, unknown>)) {
    ...
    } else if (typeof v === 'number' && Math.abs(v) > 1 && Math.abs(v) < 180 && /lat|lon/i.test(k)) {
...
export function summariseResult(tool: string, result: unknown): string {
  ...
  switch (tool) {
    ...
    case 'compare_ucsc_options': {
      ...
      const opts = (r.options as unknown as { routeId: string; feasible: boolean }[]) ?? [];
      return best
    ...
    case 'analyze_route_evidence': {
      const e = r.evidence as unknown as { label?: string; confidence?: number };
  ...
  const prov =
    args.result && typeof args.result === 'object'
      ? ((args.result as Record<string, unknown>).provenance as
          | { source?: string; origin?: string; observedAtIso?: string | null; freshness?: { ageSeconds?: number } }
          | undefined)
      : undefined;
