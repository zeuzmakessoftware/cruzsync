...
function makeContext(input: RunAgentInput): ToolContext {
  const places = resolvePlacesProvider({
    demo: input.demo,
    nowMs: input.nowMs,
  });
  return {
...
async function runDeterministic(
  ...
  if (input.direction === "to-campus") {
    ...
    if (!input.alreadyDowntown) {
      const leg = (await call("build_multileg_trip", {
        destinationKey: dest,
      })) as { earliestAtArea1Iso?: string } | undefined;
    ...
    await call("recommend_next_action", {
      direction: "to-campus",
      destinationKey: dest,
    });
  } else {
    ...
    const availableMinutes = nextDeparture
      ? Math.max(
          0,
          Math.round((Date.parse(nextDeparture) - ctx.nowMs) / 60_000),
        )
      : 30;
...
function composeDeterministicMessage(
  ...
  const rec = toolResults.find((t) => t.tool === "recommend_next_action")
    ?.result as Record<string, never> | undefined;
  const cmp = toolResults.find((t) => t.tool === "compare_ucsc_options")
    ?.result as Record<string, never> | undefined;
  const places = toolResults.find((t) => t.tool === "get_nearby_wait_places")
    ?.result as Record<string, never> | undefined;

  if (!rec)
    return "No recommendation could be produced from the available data.";

  const lines: string[] = [];
  lines.push(
    `${rec.headline as unknown as string}. ${rec.subhead as unknown as string}.`,
  );
  ...
  if (cmp?.options) {
    ...
    const parts = opts.map((o) => {
      if (!o.feasible)
        return `the ${o.routeId} is out (${o.blockedReasons[0] ?? "not feasible"})`;
    ...
    if (parts.length)
      lines.push(`Comparing the campus routes: ${parts.join("; ")}.`);
  }
  ...
  if (rec.leaveByIso) {
    lines.push(
      `Leave by ${formatClock(Date.parse(rec.leaveByIso as unknown as string))}.`,
    );
  }
  ...
  if (snapshot.origin === "fixture") {
    lines.push(
      "All vehicle data above is labelled demonstration data, not a live observation.",
    );
  } else if (
    snapshot.origin === "cache" ||
    snapshot.freshness.label !== "fresh"
  ) {
    lines.push(
...
async function runGemma(
  ...
  const contents: GeminiContent[] = [
    ...
    { role: "user", parts: [{ text: buildSystemPrompt() }] },
    {
      ...
      parts: [
        {
          text: "Understood. I will call tools for every number and never do the arithmetic myself.",
        },
      ],
    },
    {
      ...
      parts: [
        {
          text: [
            ...
            input.destinationKey
              ? `Campus destination: ${input.destinationKey}`
              : "",
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    for (const part of calls) {
      ...
      if (outcome.ok)
        toolResults.push({ tool: fc.name, result: outcome.result });
  ...
  if (cfg.gemmaLive && cfg.googleApiKey) {
    try {
      ...
      return {
        ...
        trace: out.trace,
        recommendation: out.toolResults.find(
          (t) => t.tool === "recommend_next_action",
      ...
      return {
        ...
        trace: det.trace,
        recommendation: det.toolResults.find(
          (t) => t.tool === "recommend_next_action",
        )?.result as Record<string, unknown> | undefined,
