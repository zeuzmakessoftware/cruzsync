...
const rank = (
  places: WaitPlace[],
  filters: PlaceFilters = NO_FILTERS,
  nowMs = at(20, 12),
) =>
  rankWaitPlaces({
    places,
...
describe("opening hours parsing", () => {
  ...
  it("refuses to guess at expressions it does not fully understand", () => {
    for (const expr of [
      ...
      expect(
        parseOpeningHours(expr, MONDAY, "openstreetmap", 0).parsed,
        expr,
      ).toBe(false);
    }
  });
    ...
    const p = mapOverpassElement(
      {
        ...
        lat: 36.97,
        lon: -122.02,
        tags: { name: "Anon Café", amenity: "cafe" },
      },
      Date.now(),
    )!;
    ...
    const p = mapOverpassElement(
      {
        type: "way",
        id: 3,
        center: { lat: 36.97, lon: -122.02 },
        tags: { name: "A Park", leisure: "park" },
      },
      Date.now(),
    )!;
    ...
    const p = mapOverpassElement(
      {
        ...
        lat: 36.97,
        lon: -122.02,
        tags: { name: "Chain", amenity: "cafe", brand: "Chain" },
      },
      Date.now(),
    )!;
...
describe("feasibility gating", () => {
  ...
  it("never marks a place with unverifiable hours as safe to visit", () => {
    const [c] = rank([
      place({ hours: parseOpeningHours(null, MONDAY, "openstreetmap", 0) }),
    ]);
  ...
  it("an unparseable hours string is treated as unknown, not as open", () => {
    const [c] = rank([
      place({
        hours: parseOpeningHours("sunrise-sunset", MONDAY, "openstreetmap", 0),
      }),
    ]);
    expect(c.feasible).toBe(false);
  ...
  it("drops permanently closed venues entirely", () => {
    expect(
      rank([place({ businessStatus: "CLOSED_PERMANENTLY" })]),
    ).toHaveLength(0);
  });
...
describe("rider filters", () => {
  ...
  it("an unknown amenity warns instead of silently passing or silently failing", () => {
    const [c] = rank([place({ hasWifi: "unknown" })], {
      ...NO_FILTERS,
      requireWifi: true,
    });
  ...
  it("a satisfied requirement passes cleanly", () => {
    const [c] = rank([place({ hasWifi: true })], {
      ...NO_FILTERS,
      requireWifi: true,
    });
    expect(c.feasible).toBe(true);
...
describe("ranking integrity", () => {
  ...
  it("the stay-near-stop fallback explains itself", () => {
    const advice = stayNearStopAdvice(
      RIVERFRONT.AREA_2.label,
      "Hours could not be verified.",
    );
...
describe("the committed places fixture", () => {
  it("contains real downtown venues captured from OpenStreetMap", () => {
    ...
    expect(places.some((p) => p.name.includes("Bookshop Santa Cruz"))).toBe(
      true,
    );
  });
  ...
  it("re-resolves hours for the day being asked about, not the capture day", () => {
    ...
    const monday = getFixturePlaces(at(20, 12)).find((p) =>
      p.name.includes("Mariposa"),
    );
    const tuesday = getFixturePlaces(
      serviceDateTimeToEpochMs("20260721", 20 * 3600),
    ).find((p) => p.name.includes("Mariposa"));
