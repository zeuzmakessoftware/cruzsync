...
import { analyzeRouteEvidence, findRelevantAlerts, isAlertActive } from '@/lib/engine/evidence';
...
describe('route evidence scoring', () => {
  ...
  it('never describes a missing vehicle as a cancellation', () => {
    ...
    const text = [...ev.caveats, ...ev.signals.map((s) => s.detail)].join(' ').toLowerCase();
  ...
  it('reports an agency-published cancellation as exactly that, and nothing less', () => {
    ...
    expect(ev.caveats.join(' ')).toContain('reported cancellation rather than an inference');
  });
...
describe('service alerts', () => {
  ...
  it('an expired alert is not applied', () => {
    const past = alert({ activePeriods: [{ startMs: now - 7200_000, endMs: now - 3600_000 }] });
  ...
  it('an alert for a different route does not affect this one', () => {
    const snap = { ...emptySnapshot(now), alerts: [alert({ informedRouteIds: ['72'] })] };
...
describe('comparing Routes 11, 18 and 19', () => {
  ...
  it('the saved Route 11 preference cannot override a genuinely better option', () => {
    ...
    const pref = eleven?.scoreBreakdown.find((b) => b.factor === 'rider_preference');
  ...
  it('extra transfer buffer makes tight connections drop out', () => {
    ...
    const cautiousTrip = cautious.options.find((o) => o.routeId === '18')?.tripId;
...
describe('multi-leg journey', () => {
  ...
  it('reduced mobility lengthens the walk and shrinks the margin', () => {
    ...
    expect(slower.downtownTransferMarginSec!).toBeLessThan(base.downtownTransferMarginSec!);
  });
...
describe('safe wait calculation', () => {
  ...
  it('fires the wrap-up nudge before leave-by', () => {
    const r = calculateSafeWait({ nowMs: now, predictedDepartureMs: departure, walkSeconds: 240 });
  ...
  it('a longer walk always reduces usable time', () => {
    const near = calculateSafeWait({ nowMs: now, predictedDepartureMs: departure, walkSeconds: 120 });
    const far = calculateSafeWait({ nowMs: now, predictedDepartureMs: departure, walkSeconds: 600 });
...
describe('uncertainty buffer grows when confidence falls', () => {
  ...
  it('weak evidence buys more padding than strong evidence', () => {
    const strong = deriveUncertaintyBuffer({ evidence: mk('outbound-11-wins', '2147020') });
    const weak = deriveUncertaintyBuffer({ evidence: mk('outbound-11-ghost', '2147020') });
  ...
  it('a stale feed increases the buffer', () => {
    expect(deriveUncertaintyBuffer({ feedAgeSeconds: 600 }).seconds).toBeGreaterThan(
      deriveUncertaintyBuffer({ feedAgeSeconds: 5 }).seconds,
    );
  });
});
...
describe('walking estimates', () => {
  ...
  it('applies a detour factor and flags itself as an estimate', () => {
    ...
    expect(w.metres).toBeGreaterThan(haversineMetres(RIVERFRONT.AREA_2, RIVERFRONT.AREA_1));
  });

  it('reduced mobility lengthens the estimate', () => {
    const a = estimateWalkSeconds(RIVERFRONT.AREA_2, RIVERFRONT.AREA_1);
    const b = estimateWalkSeconds(RIVERFRONT.AREA_2, RIVERFRONT.AREA_1, { reducedMobility: true });
