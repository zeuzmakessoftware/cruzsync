...
describe("tool schema surface", () => {
  ...
  it("rejects malformed arguments rather than coercing them", () => {
    expect(
      toolArgSchemas.calculate_safe_wait.safeParse({
        predictedDeparture: "soon",
        walkSeconds: 10,
      }).success,
    ).toBe(false);
    expect(
      toolArgSchemas.get_stop_schedule.safeParse({
        stopId: "1466",
        timeWindowMinutes: 99999,
      }).success,
    ).toBe(false);
    expect(
      toolArgSchemas.get_nearby_wait_places.safeParse({
        boardingStopId: "1466",
      }).success,
    ).toBe(false);
  });
...
describe("tool execution validates both directions", () => {
  ...
  it("a valid call passes its own result schema", async () => {
    ...
    expect(
      toolResultSchemas.compare_ucsc_options.safeParse(r.result).success,
    ).toBe(true);
  });
  ...
  it("every tool result carries provenance the model can cite", async () => {
    ...
    for (const [name, args] of [
      ...
      const prov = (
        r.result as { provenance: { source: string; origin: string } }
      ).provenance;
  ...
  it("evidence payloads always declare that confidence is uncalibrated", async () => {
    ...
    const options = (
      r.result as {
        options: { evidence?: { confidenceIsCalibrated: boolean } }[];
      }
    ).options;
  ...
  it("a headway is only computed when a direction is given", async () => {
    ...
    expect(
      (directed.result as { headway: { maxGapMinutes: number } }).headway
        .maxGapMinutes,
    ).toBe(60);
  });
});
...
describe("trace sanitisation", () => {
  ...
  it("redacts free text the rider typed", () => {
    const out = redactArgs({
      message: "I live at 42 Elm Street and I am late",
    });
  ...
  it("recurses into nested objects", () => {
    const out = redactArgs({
      filters: { apiKey: "nested-secret", requireQuiet: true },
    });
  ...
  it("a trace entry never contains a reasoning or thinking field", () => {
    const entry = buildTraceEntry({
      ...
      result: {
        count: 0,
        provenance: { source: "x", origin: "fixture", observedAtIso: null },
      },
    });
...
describe("the system prompt states the guardrails explicitly", () => {
  ...
  it("forbids claiming affiliation with the agency", () => {
    expect(prompt).toMatch(
      /affiliation with or endorsement by Santa Cruz METRO/i,
    );
  });
...
describe("provider configuration is honest about what it can do", () => {
  it("only advertises Gemma models the Gemini API actually serves", () => {
    ...
    expect(SUPPORTED_GOOGLE_GEMMA_MODELS as readonly string[]).not.toContain(
      "gemma-4-12b-it",
    );
...
describe("end-to-end happy path", () => {
  ...
  it("the comparison and the recommendation always agree on the winner", async () => {
    for (const sceneId of ["outbound-11-wins", "outbound-11-ghost"] as const) {
      ...
      const cmp = result.toolResults.find(
        (t) => t.tool === "compare_ucsc_options",
      )?.result as {
        bestRouteId: string | null;
      };
      expect(`TRANSFER TO ${cmp.bestRouteId}`, sceneId).toBe(
  ...
  it("never asserts a cancellation anywhere in the rider-facing output", async () => {
    for (const sceneId of [
      ...
      expect(result.message.toLowerCase(), sceneId).not.toMatch(
        /\bis cancelled\b|\bwas cancelled\b|\bnot running\b/,
      );
    }
  });
  ...
  it("reports honestly when the destination eliminates most routes", async () => {
    ...
    expect(
      cmp.options.find((o) => o.routeId === "11")!.blockedReasons.join(" "),
    ).toMatch(/does not serve/);
  });
});
